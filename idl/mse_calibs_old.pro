...
IF N_PARAMS(0) LE 1 OR N_ELEMENTS(SHOT) NE 1 THEN BEGIN
  ...
  MESSAGE,/CONT,'                    ABMODE,GAIN,SLOPE,BSCALE,BOFFSET,R,Z,IERR'
  RETURN
ENDIF

MXCH=36L
NCHANS=0L
OSHOT=LONG(SHOT) & IERR=0L & ABMODE=LONARR(MXCH)
GAIN=FLTARR(MXCH) & SLOPE=GAIN & BSCALE=GAIN & BOFFSET=GAIN & R=GAIN & Z=GAIN
