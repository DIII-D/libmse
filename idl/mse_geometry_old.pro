PRO MSE_GEOMETRY,CHANNEL,R,Z,A1,A2,A3,A4,A5,A6,VPORT,SIGHTLINE,ABMODE,LRMODE,$
   ...
   IF N_PARAMS(0) LE 1 OR N_ELEMENTS(CHANNEL) EQ 0 THEN BEGIN
      ...
      ENDIF
      PRINT,'where:'
      PRINT,'  CHANNEL    : input   channel number, 1 to 8, or array of chan nums'
   ...
   TMPR=0.0 & TMPZ=0.0 & TMPA1=0.0 & TMPA2=0.0 & TMPA3=0.0 & TMPA4=0.0
   TMPA5=0.0 & TMPA6=0.0
   TMPSIGHTLINE=0L & TMPABMODE=0L & TMPLRMODE=0L & TMPVPORT=0L
...
;      ENDIF
      IF C EQ 0 THEN BEGIN
         R=FLTARR(NCH) & Z=R & A1=R & A2=R & A3=R & A4=R &  A5=R & A6=R
         SIGHTLINE=R & ABMODE=R & LRMODE=R & VPORT=R
      ENDIF
      R(C)=TMPR & Z(C)=TMPZ & A1(C)=TMPA1 & A2(C)=TMPA2 & A3(C)=TMPA3 & A4(C)=TMPA4
      A5(C)=TMPA5 & A6(C)=TMPA6 & VPORT(C)=TMPVPORT
      SIGHTLINE(C)=TMPSIGHTLINE & ABMODE(C)=TMPABMODE & LRMODE(C)=TMPLRMODE

   ENDFOR
   IF ISASCALAR(CHANNEL) THEN BEGIN
      R=R(0) & Z=Z(0) & A1=A1(0) & A2=A2(0) & A3=A3(0) & A4=A4(0)
      A5=A5(0) & A6=A6(0) & VPORT=VPORT(0)
      SIGHTLINE=SIGHTLINE(0) & ABMODE=ABMODE(0) & LRMODE=LRMODE(0)
   ENDIF
